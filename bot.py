...
STR = {
    "no_access":
    "⛔ No access. Please enter your key via /start.",
    "access_invalid":
    "⛔ Access invalid: {msg}\nSend a new key.",
    "cooldown":
    "⏳ Please wait {remaining}s before using /scan again (anti-spam).",
    "no_pairs":
    ...
    "⚠️ Invalid action.",
    "session_expired":
    "⌛ Session expired. Run /scan again.",
    "enter_key":
    "Please enter your access key:",
    "no_active_access":
    "⛔ No active access. Send your key or use /start.",
    "key_unlinked":
    "✅ Key unlinked. Send a new key or /start.",
    "usage_token":
    "Usage: `/token <mint | birdeye/solscan link | SYMBOL (MINT)>`",
    "cant_detect_mint":
    "❌ Can't detect mint address. Send a Solana mint or a direct link to Birdeye/Solscan.",
    "fetching_data":
    "Fetching token data…\n`{mint}`",
    "no_data":
    "No data",
    ...
    "Usage: `/fav del <mint>`",
    "fav_removed":
    "✅ {mint} removed from favorites.",
    "fav_not_found":
    "❌ {mint} not in your favorites.",
    "btn_fav_remove":
    ...
    "📤 Share",
    "info_fdv":
    "FDV (Fully Diluted Valuation) = token price × total supply. Shows potential market cap if all tokens were in circulation.",
    "info_lp":
    ...
    "😕 No fresh pairs available from any source.\nTry `/token <mint>` instead.",
    "chain_current":
    "Current chain: {chain}",
    "chain_set":
    "✅ Chain set to: {chain}",
    "chain_invalid":
    "❌ Invalid chain. Use: sol, eth, or bsc",
    "chain_usage":
    "Usage: `/chain <sol|eth|bsc>` or `/chain` to see current",
    "chain_not_supported":
    "⚠️ {chain} support coming soon. Only Solana (sol) is currently available.\nUse `/chain sol` to switch back.",
    "research_menu":
    ...
    "filter_invalid":
    "❌ Invalid value. Try again or send /skip",
    "filters_cleared_all":
    "✅ All filters cleared",
    "filters_active":
    "📊 Active filters:\n{filters}",
...
def main_menu_keyboard() -> ReplyKeyboardMarkup:
    ...
    return ReplyKeyboardMarkup(keyboard=[
        [
            ...
            KeyboardButton(text="🎯 Find Token"),
...
def scan_nav_kb(sid: str, idx: int, mint: str,
    ...
    row_buy_chart = [
        InlineKeyboardButton(text="📊 Dexscreener", url=dexscreener_link),
        InlineKeyboardButton(text="💧 Raydium", url=raydium_link),
    ]
    row_links = [
...
async def alerts_handler(m: Message):
    ...
    if sub == "del":
        if len(args) >= 3:
            ...
            if not row or not row[0]:
                await m.answer(f"❌ No alerts found for {mint[:8]}...",
                               **MSG_KW)
                conn.close()
                return
